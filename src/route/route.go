...
func AppRoute(h *server.Hertz) {
	...
	h.GET("/v1/stat/category", apc("ledger:transaction:list"), statistic.CategoryPie())       // 收支分类构成
	h.GET("/v1/stat/expense-rank", apc("ledger:transaction:list"), statistic.ExpenseRank())   // 本月支出分类TOP排行
}
